...
function getOptionalContactDetail(fields, key, label) {
  const value = String(fields?.[key] || '').trim();
  return value ? `${label}: ${value}` : '';
}

function normalizeContactFields(fields = {}) {
  const servizio = String(fields.servizio || '').trim();
  const nome = String(fields.nome || '').trim();
  const email = String(fields.email || '').trim();
  const telefono = String(fields.telefono || '').trim();
  const luogo = String(fields.luogo || fields.comune || '').trim();
  const rawMessage = String(fields.messaggio || fields.message || '').trim();
  const details = [
    getOptionalContactDetail({ servizio }, 'servizio', 'Servizio'),
    getOptionalContactDetail({ luogo }, 'luogo', 'Luogo'),
  ].filter(Boolean);
  const messaggio = [rawMessage, details.length ? `\n${details.join('\n')}` : ''].join('').trim();

  return { servizio, nome, email, telefono, luogo, messaggio };
}
...
async function handleContactRequest(fields) {
  const { servizio, nome, email, telefono, luogo, messaggio } = normalizeContactFields(fields);
  ...
  const safeServizio = escapeHtml(servizio || 'Non specificato');
  const safeNome = escapeHtml(nome);
  const safeEmail = escapeHtml(email);
  const safeTelefono = escapeHtml(telefono || 'Non fornito');
  const safeLuogo = escapeHtml(luogo || 'Non fornito');
  const safeMessaggio = escapeHtml(messaggio).replace(/\n/g, '<br>');

  const notifyHtml = `
    <h2 style="color:#203863;">Nuova richiesta dal sito HURKA!</h2>
    <table style="border-collapse:collapse;width:100%;max-width:600px;font-family:sans-serif;">
      <tr><td style="padding:10px 12px;border:1px solid #e0e0e0;font-weight:bold;background:#f5f5f5;">Servizio</td><td style="padding:10px 12px;border:1px solid #e0e0e0;">${safeServizio}</td></tr>
      <tr><td style="padding:10px 12px;border:1px solid #e0e0e0;font-weight:bold;background:#f5f5f5;">Nome</td><td style="padding:10px 12px;border:1px solid #e0e0e0;">${safeNome}</td></tr>
      <tr><td style="padding:10px 12px;border:1px solid #e0e0e0;font-weight:bold;background:#f5f5f5;">Email</td><td style="padding:10px 12px;border:1px solid #e0e0e0;"><a href="mailto:${safeEmail}">${safeEmail}</a></td></tr>
      <tr><td style="padding:10px 12px;border:1px solid #e0e0e0;font-weight:bold;background:#f5f5f5;">Telefono</td><td style="padding:10px 12px;border:1px solid #e0e0e0;">${safeTelefono}</td></tr>
      <tr><td style="padding:10px 12px;border:1px solid #e0e0e0;font-weight:bold;background:#f5f5f5;">Luogo</td><td style="padding:10px 12px;border:1px solid #e0e0e0;">${safeLuogo}</td></tr>
      <tr><td style="padding:10px 12px;border:1px solid #e0e0e0;font-weight:bold;background:#f5f5f5;">Messaggio</td><td style="padding:10px 12px;border:1px solid #e0e0e0;">${safeMessaggio}</td></tr>
    </table>`;
  ...
  return res(200, {
    success: true,
    message: 'Richiesta inviata con successo! Controlla la tua email per la conferma.',
  });
}
...
export async function handler(event) {
  ...
  try {
    ...
    const contentType = getHeader(event.headers, 'content-type');
    if (contentType.includes('multipart/form-data')) {
      if (event?.queryStringParameters?.inboundToken) {
        return await handleInboundEmail(event);
      }
      const fields = parseMultipartFormData(decodeBody(event), contentType);
      return await handleContactRequest(fields);
    }

    const body = typeof event.body === 'string' ? JSON.parse(event.body) : event.body;
    return await handleContactRequest(body || {});
  } catch (err) {
